...
J'ai d'abord examiné la structure de la database en interface graphique avec une extension VSCODE pour voir les tables et les colonnes.

J'ai affiché tous les rapports de scène de crime avec :
...
- SELECT sert à sélectionner quelles colonnes afficher.
  - Ici, on utilise la wildcard \* pour afficher toutes les colonnes.
...
Ça affichait trop de données pour que ça soit exploitable.
J'ai décidé d'afficher les meurtres ayant été commis à SQL City.
...
Le troisième résultat semble être le plus pertinent.
...
- WHERE indique de n'afficher que les données respectant les conditions.
  ...
  - Le numéro de rue doit être le plus grand numéro de rue existant sur la Northwestern Dr.
    - MAX renvoie le plus grand élément des résultats.
...
- WHERE indique de n'afficher que les données respectant les conditions.
  ...
  - Le prénom de la personne doit être Annabel.
    - LIKE sert à vérifier qu'un texte est sous une certaine forme, ici, Annabel puis un nombre indéterminé de caractères.
...
On a beaucoup d'informations sur le meurtrier :
...
- WHERE indique de n'afficher que les données respectant les conditions.
  ...
  - L'id de membre doit commencer par 48Z.
    - LIKE sert à vérifier qu'un texte soit sous une certaine forme, ici, 48Z puis un nombre indéterminé de caractères.
...
- FROM indique dans quelle table on va chercher les informations.
- WHERE indique de n'afficher que les données respectant les conditions.
  - La plaque contient H42W.
    - LIKE sert à vérifier qu'un texte soit sous une certaine forme, ici, H42W entre n'importe quels caractères.
...
On peut réduire ces requêtes en une :
...
On va trouver la personne ayant commandité le meurtre en se basant sur ces informations.
...
- WHERE indique de n'afficher que les données respectant les conditions.
  ...
  - Le modèle de la voiture est Model S.
  - L'id de la personne apparait trois fois dans des lignes de la table facebook_event_checkin où le nom de l'événement est SQL Symphony Concert et la date est comprise entre le 01/12/2017 et le 31/12/2017.
    - IN s'assure que la valeur de la colonne spécifiée est une des valeurs données en argument.
    - GROUP BY permet de grouper les résultats ayant la même valeur dans une colonne.
